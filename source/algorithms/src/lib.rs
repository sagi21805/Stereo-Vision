...
mod tests {
    ...
    use crate::integral_image::{
        self, integral_image::rectangle_sum, integral_image_neon::neon_integral_image,
    };
    ...
    fn test_integral_sum() {
        ...
        let integral_image = neon_integral_image(source_image.as_slice(), width, height);
        ...
        let sum = rectangle_sum(&integral_image, width + 1, x, y, rect_width, rect_height);
        assert_eq!(sum, 34);
    }

    #[test]
    fn black_white() {
        let source_image: Vec<u8> = vec![0, 255, 0, 0, 0, 0, 255, 255, 0, 255, 0, 0, 0, 0, 0, 255];
